	...
	function mkPrototypeMethod(method, key) {
		...
		return function wrapperMethod() {
			...
			if(args.length == 3  && typeof args[args.length-1] != "function" ) {
				throw new Error('last Argument is not a callback');
				pool._avail(client);
				return pool;
			} else if (args.length == 2 && typeof args[args.length-1] != "function" ) {
				args.push( function() {} )
			} else if( ( args.length == 3 || args.length == 2)  && typeof args[args.length-1] == "function"){

			} else {
				throw new Error('not enoth arguments');
				pool._avail(client);
				return pool;
			}
